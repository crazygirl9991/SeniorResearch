...
public class DisplayFrame implements ActionListener {
	...
	public void display() throws Exception {
		...
		SpectrumPlotter plot1 = new SpectrumPlotter( new TableElement[] {te1, te2} );
		panel.add(plot1);
		
		// ...and another plot of the ratio between them
		SpectrumPlotter plot2 = new SpectrumPlotter( new TableElement[] {calculateRatio(te1, te2)} );
		panel.add(plot2);
	...
	private TableElement calculateRatio(TableElement element1, TableElement element2) {
		...
		int n = Math.min(element1.getSpectrumDataX().length, element2.getSpectrumDataX().length);
		...
		for(int i = 0; i < n; i++) {
			...
			ratioY[i] = element1.getSpectrumDataY()[i] / element2.getSpectrumDataY()[i];
		}
